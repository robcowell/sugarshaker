...
music_dma:	equ	1					;Play DMA-sample sequence music (STe only and need sys/loader.s included)
...
stlow_dblbuf:	equ	1					;
stlow_tribuf:	equ	0				;Need sys/c2p.s included
fullscr_st:	equ	0
fullscr_ste:	equ	0				;Need sys/fade.s included
syncscrl:	equ	0				;Need sys/sscrl.s and sys/fade.s included
...
;-------------- User demopart includes
	...
	ifne	stlow_dblbuf
		include	'stlow.dbl/dmg1.s'
		include 'stlow.dbl/dmg2.s'
		include 'stlow.dbl/dmg3b.s'
		include 'stlow.dbl/dmg4.s'
		include 'stlow.dbl/dmg5.s'
		include 'stlow.dbl/dmg6.s'
	endc
...
initlist:
	ifne	stlow_dblbuf
		jsr	dmg1_init			;Do inits before demo begin
	endc
...
;------------- ST-LOW double buffer example
	ifne	stlow_dblbuf
		dc.l	10,0,0,dummy,dummy,dmg1_runtime_init			;Do inits needed while demo runs
		dc.l	210,0,0,dmg1_vbl,dummy,dmg1_main				;Run demopart 1 - bouncing logo
		dc.l 	10,0,0,dummy,dummy,dmg1_exit					;Tidy up timer b and palette

		dc.l	10,0,0,dummy,dummy,dmg2_runtime_init			;Do inits needed while demo runs
		dc.l	690,0,0,dmg2_vbl,dummy,dmg2_main				;Run demopart 2 - scroller

		dc.l	10,0,0,dummy,dummy,dmg3_runtime_init			;Do inits needed while demo runs
		dc.l	750,0,0,dmg3_vbl,dummy,dmg3_main				;Run demopart 3

		dc.l	10,0,0,dummy,dummy,dmg4_runtime_init			;Do inits needed while demo runs
		dc.l	1000,0,0,dmg4_vbl,dummy,dmg4_main				;Run demopart 4 - Goat plasma
		dc.l 	30,0,0,dummy,dummy,dmg4_exit					;Tidy up timer b and palette

		dc.l	10,0,0,dummy,dummy,dmg5_runtime_init			;Do inits needed while demo runs
		dc.l	1000,0,0,dmg5_vbl,dummy,dmg5_main				;Run demopart 5 - DNA twister
...
;------------- Fullscreen example
	ifne	fullscr_st
		...
		dc.l	960,97,4,fullscr_st_vbl,fullscr_st_ta,fullscr_st_main
	endc

;------------- Optimized fullscreen example (STe only)
	ifne	fullscr_ste
		dc.l	30,0,0,dummy,dummy,fullscr_ste_runtime_init
		dc.l	5000,97,4,fullscr_ste_vbl,fullscr_ste_ta,fullscr_ste_main
	endc
...
;------------- Syncscroll in fullscreen example
	ifne	syncscrl_fs
		...
		dc.l	525,97,4,syncscrl_fs_vbl,syncscrl_fs_ta,syncscrl_fs_main
	endc
...
;------------- Spectrum 512 displayer example
	...
	endc
		...
		dc.l	20,0,0,dummy,dummy,dmg6_runtime_init			
		dc.l	5000,0,0,dmg6_vbl,dummy,dmg6_main
